...
func TestEncryptionIntegration(tt *testing.T) {
	...
	verifyKMSPlugins := func() {
		...
		for keyID := range expectedKeyIDs {
			...
			if !ok {
				t.Fatalf("expected kms-plugin-config for keyID %s but not found in encryption-config secret", keyID)
			}
		...
		for keyID := range cfg.KMSPlugins {
			if !expectedKeyIDs[keyID] {
				t.Fatalf("unexpected kms-plugin-config for keyID %s in encryption-config secret", keyID)
			}
		}
	}
	...
	require.NoError(t, err)
	require.Equal(t, "VaultKMSConfig", pluginConfig.GetKind())
	require.Equal(t, vaultKMSConfigPrimary, pluginConfig.GetName())
	pluginImage, err := kms.PluginConfigStatusString(pluginConfig, "kmsPluginImage")
	require.NoError(t, err)
	require.Equal(t, kmsPluginImageA, pluginImage)
	pluginConfigVaultAddress, err := kms.PluginConfigSpecString(pluginConfig, "vaultAddress")
	require.NoError(t, err)
	require.Equal(t, "https://vault.example.com", pluginConfigVaultAddress)
	pluginConfigVaultKeyPath, err := kms.PluginConfigSpecString(pluginConfig, "vaultKeyPath")
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, vaultKMSConfigNew, pluginConfig13.GetName())
	pluginConfig13VaultAddress, err := kms.PluginConfigSpecString(pluginConfig13, "vaultAddress")
	require.NoError(t, err)
	require.Equal(t, "https://vault-new.example.com", pluginConfig13VaultAddress)
	pluginConfig13VaultKeyPath, err := kms.PluginConfigSpecString(pluginConfig13, "vaultKeyPath")
	require.NoError(t, err)
