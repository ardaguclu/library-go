	...
	scenarios := []struct {
			...
			input: encryptiondatatesting.CreateEncryptionCfgWithWriteKey([]encryptiondatatesting.EncryptionKeysResourceTuple{
				{
					Resource: "secrets",
					Keys: []apiserverconfigv1.Key{
						{Name: "1", Secret: "AAAAAAAAAAAAAAAAAAAAAA=="},
					},
					Modes: []string{"KMS"},
				},
			}),
		...
		// scenario 12
		{
			...
			output: map[schema.GroupResource]state.GroupResourceState{
				{Group: "", Resource: "secrets"}: {
					WriteKey: state.KeyState{
						Key:  apiserverconfigv1.Key{Name: "2", Secret: "AAAAAAAAAAAAAAAAAAAAAA=="},
						Mode: "KMS",
					},
					ReadKeys: []state.KeyState{
						{
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			if !cmp.Equal(expectedOutput, actualOutput.Encryption.Resources) {
				t.Fatal(fmt.Errorf("%s", cmp.Diff(expectedOutput, actualOutput.Encryption.Resources)))
			}
		})
	}
}
