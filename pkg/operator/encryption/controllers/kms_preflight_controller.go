...
//     return an error. Otherwise requeue and wait. If the pod has exceeded
//     the startup timeout (3m) without reporting, return an error with the
//     reason the pod is stuck (e.g. ImagePullBackOff, Pending).
//
...
func (c *kmsPreflightController) runPreflightChecks(ctx context.Context) (requeue bool, progressReason, progressMessage string, err error) {
	requiredHash, existingResult, kmsCfg, pluginConfig, err := c.preflightRequired(ctx)
	if err != nil {
		return false, "", "", err
	}
	...
	if apierrors.IsNotFound(err) {
		...
		encryptionConfig, err := c.encryptionConfigurationComputer.ComputeEncryptionConfiguration(ctx, kmsCfg, pluginConfig)
		if err != nil {
...
func (c *kmsPreflightController) preflightRequired(ctx context.Context) (string, *operatorv1.KMSPreflightResult, *configv1.KMSPluginConfig, *unstructured.Unstructured, error) {
	apiServer, err := c.apiServerClient.Get(ctx, "cluster", metav1.GetOptions{})
	if err != nil {
		return "", nil, nil, nil, fmt.Errorf("failed to get apiserver config: %w", err)
	}
	...
	if err != nil {
		return "", nil, nil, nil, fmt.Errorf("failed to get KMS encryption status: %w", err)
	}
	...
	resolvedConfig, err := ResolveKMSConfig(ctx, c.dynamicClient, apiServer.Spec.Encryption.KMS)
	if err != nil {
		return "", nil, nil, nil, err
	}
	providerCfg, err := newKMSProviderConfig(apiServer.Spec.Encryption.KMS.Type, resolvedConfig)
	if err != nil {
		return "", nil, nil, nil, fmt.Errorf("failed to create KMS provider config: %w", err)
	}
	...
	if err != nil {
		return "", nil, nil, nil, fmt.Errorf("failed to create KMS config hasher: %w", err)
	}
	currentHash, err := hasher.hash(ctx)
	if err != nil {
		return "", nil, nil, nil, fmt.Errorf("failed to compute KMS config hash: %w", err)
	}
	...
	return requiredHash, &encryptionStatus.Preflight.Result, &apiServer.Spec.Encryption.KMS, resolvedConfig, nil
}
