...
func TestStateController(t *testing.T) {
	scenarios := []struct {
		...
		// it also checks the content and the order of encryption providers, this test expects identity first and aescbc second
		{
			...
			expectedEncryptionCfg: encryptiondatatesting.CreateEncryptionCfgNoWriteKey("1", "NjFkZWY5NjRmYjk2N2Y1ZDdjNDRhMmFmOGRhYjY4NjU=", "secrets"),
			validateFunc: func(ts *testing.T, actions []clientgotesting.Action, destName string, expectedEncryptionCfg *encryptiondata.Config) {
		...
		// scenario 3
		{
			...
			expectedEncryptionCfg: func() *encryptiondata.Config {
		...
		// scenario 11
		{
			...
			initialResources: []runtime.Object{
				encryptiontesting.CreateDummyKubeAPIPod("kube-apiserver-1", "kms", "node-1"),
				func() *corev1.Secret { // encryption config in kms namespace
					ecs := createEncryptionCfgSecret(t, "kms", "1", &encryptiondata.Config{Encryption: &apiserverconfigv1.EncryptionConfiguration{}})
					ecs.Data[encryptiondata.EncryptionConfSecretName] = []byte{1, 2, 3} // invalid
					return ecs
				}(),
			},
		...
		// scenario 12: KMS secret exists => encryption config created without write key
		{
			name:                     "KMS: secret with EncryptionConfig is created without a write key",
			targetNamespace:          "kms",
			encryptionSecretSelector: metav1.ListOptions{LabelSelector: "encryption.apiserver.operator.openshift.io/component=kms"},
			targetGRs: []schema.GroupResource{
				{Group: "", Resource: "secrets"},
		...
		// scenario 13: migrated KMS key => encryption config with KMS write key
		{
			name:            "KMS: secret with EncryptionConfig is created with KMS as write key",
			targetNamespace: "kms",
			targetGRs: []schema.GroupResource{
				{Group: "", Resource: "secrets"},
			},
		...
		// scenario 14: no-op when KMS config is stable
		{
			name:            "KMS: no-op when no key is transitioning",
			targetNamespace: "kms",
			targetGRs: []schema.GroupResource{
				{Group: "", Resource: "secrets"},
			},
		...
		// scenario 15: KMS key transitioning => new KMS write key set
		{
			name:            "KMS: new KMS key is transitioning (observed as a read key) so it is used as a write key",
			targetNamespace: "kms",
			targetGRs: []schema.GroupResource{
				{Group: "", Resource: "secrets"},
			},
		...
		// scenario 16: AESCBC→KMS migration scenario
		{
			name:            "KMS: AESCBC to KMS migration - KMS added as read key",
			targetNamespace: "kms",
			targetGRs: []schema.GroupResource{
				{Group: "", Resource: "secrets"},
			},
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			if scenario.validateFunc != nil {
				scenario.validateFunc(t, fakeKubeClient.Actions(), fmt.Sprintf("%s-%s", encryptiondata.EncryptionConfSecretName, scenario.targetNamespace), scenario.expectedEncryptionCfg)
			}
...
func validateSecretWithEncryptionConfig(actualSecret *corev1.Secret, expectedEncryptionCfg *encryptiondata.Config, expectedSecretName string) error {
	actualEncryptionCfg, err := encryptiondata.FromSecret(actualSecret)
	if err != nil {
